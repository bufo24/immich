...
export const swipeFeedback = (node: HTMLElement, options?: SwipeFeedbackOptions) => {
  ...
  let lastAssetId = options?.assetId;
  ...
  const resetPreviewContainers = () => {
    ...
    // Reset loaded flags so new previews will load on next swipe
    leftPreviewLoaded = false;
    rightPreviewLoaded = false;
  };
  ...
  let leftPreviewLoaded = false;
  let rightPreviewLoaded = false;
  ...
  const ensurePreviewCreated = (
    ...
    // Don't load the image yet - wait until swiping starts
    return preview;
  };

  const loadPreviewImage = async (url: string, img: HTMLImageElement) => {
    await imageManager.prepareImageUrl(url);
    img.src = url;
  };
  ...
  const pointerMove = (event: PointerEvent) => {
    ...
    if (!animations) {
      // Load the preview image now that swiping has started
      if (direction === 'left' && !rightPreviewLoaded && options?.rightPreviewUrl && rightPreviewImg) {
        rightPreviewLoaded = true;
        void loadPreviewImage(options.rightPreviewUrl, rightPreviewImg);
      } else if (direction === 'right' && !leftPreviewLoaded && options?.leftPreviewUrl && leftPreviewImg) {
        leftPreviewLoaded = true;
        void loadPreviewImage(options.leftPreviewUrl, leftPreviewImg);
      }
  ...
  const finishSwipe = (direction: 'left' | 'right') => {
    ...
    // Add transition before setting opacity to smooth out the asset change
  ...
  return {
    update(newOptions?: SwipeFeedbackOptions) {
      ...
      if (newOptions?.assetId && newOptions.assetId !== lastAssetId) {
        // Delay reset to allow fade transition to complete
        setTimeout(() => {
          resetPreviewContainers();
        }, 250); // Slightly longer than the 200ms fade
        lastAssetId = newOptions.assetId;
      }
      ...
      // ) {
      //   imageManager.cancelPreloadUrl(lastLeftPreviewUrl);
      // }
      // if (
      //   lastRightPreviewUrl &&
      ...
      // ) {
      //   imageManager.cancelPreloadUrl(lastRightPreviewUrl);
      // }
      options = newOptions;

      if (options?.leftPreviewUrl) {
        if (leftPreviewImg) {
          // Reset loaded flag if URL changed - will load when swiping starts
          leftPreviewLoaded = false;
        } else {
          const preview = ensurePreviewCreated(options.leftPreviewUrl, leftPreviewContainer, leftPreviewImg);
          leftPreviewContainer = preview.container;
